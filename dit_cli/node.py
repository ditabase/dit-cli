...
class Node:
    ...
    def add_extend(self, extend_expr: List[str]):
        ...
        if self == new_c:
            raise NodeError(f'Illegal recursive extension in class "{self.name}"')

        if new_c in self.extends:
            raise NodeError((f'"{self.name}" already ' f'extends "{new_c.name}"'))
    ...
    def add_attribute(self, type_expr: List[str], name: str, list_: bool = False):
        ...
        if self == new_c:
            raise NodeError(f'Illegal recursive attribution in class "{self.name}"')

        for attr in self.attrs:
            if name == attr.name:
                raise NodeError(f'"{self.name}" already has attribute "{name}"')
    ...
    def set_print(self, expr: List[str] = None, code: str = None, lang: str = None):
